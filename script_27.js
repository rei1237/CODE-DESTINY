...
function showTokenResult(code) {
    ...
    backBtn.onclick = closeResultLayer;
    ...
    const actionBtn = isSelected 
        ? `<button class="ritual-btn" style="width:100%; margin-top:20px;" onclick="closeResultLayer()">이미 선택됨 (닫기)</button>`
        : `<button class="ritual-btn" style="width:100%; margin-top:20px;" onclick="confirmSelection('${code}')">이 영혼 선택하기</button>`;

    // We need to distinguish between "Just Viewing" and "Synergy Result".
    // This function is for "Individual Reading".
    
    resContent.innerHTML = `
        <header class="res-header">
            <span class="res-icon" style="text-shadow: 0 0 30px ${getMbtiColor(code)}">${item.animal}</span>
            <h2 class="res-title">${item.title}</h2>
            <div class="res-mbti">${item.code}</div>
        </header>
        
        <div class="res-section">
            <div class="res-label">Layer 1. The Soul (성격)</div>
            <div class="res-text">${item.layer1}</div>
        </div>
        
        <div class="res-section">
            <div class="res-label">Layer 2. The Path (적성)</div>
            <div class="res-text">${item.layer2}</div>
        </div>
        
        <div class="res-section">
            <div class="res-label">Layer 3. The Heart (연애)</div>
            <div class="res-text">${item.layer3}</div>
        </div>
    ...
    // Note: This is inside the modal, so we change modal background? 
    // Or just a subtle glow. The text says "Cross-fade".
}
