...
function renderAstroInsight() {
  var birth = window._astroBirth || window._ziweiBirth || { year:2000, month:1, day:1, hour:12, minute:0, lat:37.6, lon:127.0, tz:9 };
    ...
    function _aspectName(diff){
      var defs = [
        {d:0, n:'합(Conjunction)', orb:6},
        {d:60, n:'육합(Sextile)', orb:4},
        {d:90, n:'직각(Square)', orb:5},
        {d:120, n:'삼합(Trine)', orb:5},
        {d:180, n:'충(Opposition)', orb:6}
      ];
    ...
    }).join('') || '<li>주요 5대 각(0/60/90/120/180도) 기준 유의미한 각이 오브 내에 없습니다.</li>';
  ...
  var html = '<div class="astro-body cosmic-theme star-container" id="astroBodyWrap">' + masterInsight
        ...
        +'<p style="margin-top:8px;color:#cbd5e1;">실무 프레임: MC 관련 목표 1개(브랜딩), 6H 관련 루틴 1개(생산성), 토성 관련 규율 1개(품질관리)를 90일 단위로 운영해 보세요.</p>'
        +'</div>'
        +'</div>'
    ...
    function _syBuildNarrative(meta){
      ...
      else if(score >= 70) rel = '상호성장형 인연 - 조화각이 우세하고 갈등 관리가 가능한 구조입니다.';
      else if(score >= 55) rel = '혼합형 인연 - 끌림과 긴장이 공존하며 운영 방식이 성패를 가릅니다.';
      ...
      var h78Boost = (meta.overlayNorm != null && meta.overlayNorm > 0.14)
        ...
        : (meta.overlayNorm != null && meta.overlayNorm < -0.08)
          ? '12H/6H 압력이 커서 관계 피로 관리가 핵심 과제입니다.'
      ...
      var spirit = '영적 과제는 '+meta.myElem+'-'+meta.theirElem+' 원소 결에서 드러납니다. '+h78Boost+' '
        +'점수 '+score+'/100 구간에서는 감정 조율 루틴과 갈등 복기 규칙을 함께 운영할수록 관계 성장이 빠릅니다.';
  ...
  if (isCompactView) {
    var cardsHtml = cardRows.map(function(it) {
      return '<div style="background:'+it.rowBg+';border:1px solid rgba(255,255,255,0.1);border-left:4px solid '+it.borderColor+';border-radius:10px;padding:10px 12px">'
        +'<div style="font-weight:800;color:#d8b4fe;font-size:0.84rem;line-height:1.35">'+it.icon+' '+it.pName+'</div>'
...
.gaeun-title { font-size: 0.95rem; color: #c4b5fd; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; }
  ...
  html += '<span style="font-size:0.75rem; color:#888; margin-top:8px; display:block;">궁(宮)을 클릭하면 <br>현대적 해석 리포트가 출력됩니다.</span>';
  html += '</div>';
  html += '</div>';
  html += '</div></div>';
...
var mainSt = extractMains(getPStars(pName));
      ...
      "    <div class=\"zw-report-toolbar\"><button type=\"button\" class=\"zw-report-close-btn\" onclick=\"window._closeZwDetailReport()\">리포트 닫기 ✕</button></div>" +
      "    " + contentHtml +
      "  </div>" +
      "</div>";
