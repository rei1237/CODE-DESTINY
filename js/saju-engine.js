...
function _hideLibOverlay(forceRemove) {
  ...
  _diag('overlay.lib.hidden', { forceRemove: !!forceRemove });
}
...
function loadNext(){
  ...
  _diag('cdn.try', { index: tried + 1, total: CDN_URLS.length });
...
function startSajuCalculationFlow() {
  ...
  _diag('calc.flow.start');
  if(typeof Solar==='undefined'||typeof Solar.fromYmdHms!=='function'){
    _diag('calc.flow.blocked.solar-not-ready');
  ...
  if (!overlay) {
    try { calculate(); } catch (e) {
      ...
      _diag('calc.flow.error.no-overlay', { message: e && e.message ? e.message : 'unknown' });
    }
    return;
  }
  ...
  requestAnimationFrame(() => {
    ...
    var _hardStop = setTimeout(function() {
      ...
      _diag('overlay.saju.timeout', { ms: 5000 });
    ...
    setTimeout(async () => {
      try {
        ...
        _diag('calc.failed', { message: calcErr && calcErr.message ? calcErr.message : 'unknown' });
        _showEngineFallbackNotice('콘텐츠를 불러오는 중입니다. 잠시 후 다시 시도해주세요.');
      } finally {
...
function renderAstroInsight() {
    ...
    var FIRDARIA_DAY = [
        ...
        {planet:'♀ 금성(Venus)',  years:8,  kr:'금성',  theme:'사랑·미적 성취·재물의 꽃이 활짝 피는 달콤한 시기.',
         ...
         caution:'지출이 수입을 초과하는 사치, 연애에서의 우유부단함, 감성적 판단으로 인한 비즈니스 실수를 조심하십시오. 당이나 지방 위주의 음식으로 혈당·체중 관리가 느슨해질 수 있습니다.',
        ...
        {planet:'♄ 토성(Saturn)', years:11, kr:'토성',  theme:'시련과 인내, 그 끝에 단단한 전문성이 완성되는 여정.',
         ...
         advice:'지금 어렵고 느리게 느껴지는 것이 정상입니다. 포기하지 마세요 — 지금 쌓는 전문성이 다음 10년의 거대한 자산이 됩니다. 멘토를 찾고, 장기 계획 수립에 집중하는 것이 가장 현명한 전략입니다.'},
        ...
        {planet:'♂ 화성(Mars)',   years:7,  kr:'화성',  theme:'도전·경쟁·에너지 폭발. 멈추지 말고 전진할 것.',
         detail:'화성 피르다리아는 짧지만 강렬한 7년의 전투기입니다. 경쟁·도전·개척·돌파의 에너지가 폭발하며, 행동력이 극도로 상승합니다. 스스로 움직이지 않으면 아무 일도 일어나지 않는 시기입니다. 체력·투지·직접 행동이 성과를 만들어내며, 리스크를 감수하는 과감한 결단이 요구됩니다.',
    ...
    var FIRDARIA_COMBO = {
        ...
        '수성_토성':'신중하고 정밀한 분석력이 강화됩니다. 법률·회계·전략 기획 분야에서 실력이 인정받는 시간입니다.',
        ...
        '화성_달':'감정의 화산이 폭발할 수 있습니다. 분노와 욕구를 창조적 활동이나 운동으로 승화시키는 출구 전략이 필수입니다.',
        '화성_토성':'전진하고 싶은 에너지가 현실의 벽에 부딪히는 고통스러운 국면입니다. 무리한 돌파보다 전략적 우회를 선택하세요.',
        '화성_목성':'황소처럼 돌진하고 독수리처럼 멀리 봐야 하는 시간입니다. 대담한 확장 전략이 예상치 못한 대박을 부릅니다.'
    };
    ...
    var profData = [
        ...
        },
        {
            ...
            career:'지배 행성 태양이 창조적 리더십에 힘을 실어줍니다. 공연·전시·콘테스트·경기 등 자신을 드러내는 무대에 적극적으로 뛰어드세요. 무언가 만들고 세상에 내놓는 것이 올해 최고의 전략입니다.',
        ...
        {
            theme:'건강 관리와 직업적 루틴이 미래를 좌우하는 해.',
            ...
            career:'지배 행성 수성이 분석력·데이터 처리·세밀한 기획에 힘을 줍니다. 업무 프로세스 개선, 자동화 툴 도입, CRM 정비 등 효율을 높이는 것이 성과로 이어집니다. 아르바이트·파트타임·부업으로 수입원을 다양화하기에도 좋은 해입니다.',
            love:'연인 관계에서 일상의 디테일(청결·건강·작은 배려)이 감동을 만들어냅니다. 같이 운동하거나 식단을 관리하는 루틴이 관계를 단단하게 합니다.',
        ...
        },
        {
            ...
            career:'지배 행성 화성이 비공개 전략 실행에 힘을 줍니다. 비밀 프로젝트, 은밀한 준비, 수면 아래에서의 투자가 이후 큰 결실로 드러납니다. 부채 정리·세금 관리·재무 구조 개선을 올해 안에 단행하세요.',
        ...
        },
        {
            ...
            advice:'올 한 해 반드시 정기적인 혼자만의 묵상·산책·명상 시간을 가지세요. 오래 끌어온 집착과 묵은 감정을 내려놓는 의식(편지 쓰기·일기·여행)을 해보세요. 다음 해의 새로운 출발을 위한 공간을 지금 마련하는 것이 최선의 전략입니다.'
        }
    ];
  ...
  var html = '<div class="astro-body cosmic-theme star-container" id="astroBodyWrap">' + masterInsight
    ...
    window._astroPickCeleb = function(name, birth, hour) {
        ...
        setTimeout(function() {
            try {
                var p = birth.split('-');
                /* 유명인은 시차 미상 → UTC 정오 기준 (tz=0 설정, 오차 명시) */
                var celebChart = AstroEngine.calcAll(+p[0], +p[1], +p[2], hour, 37.6, 127.0, 0);
                var celebSunIdx   = celebChart.sun.idx;
                var celebMoonIdx  = celebChart.moon.idx;
                var celebVenusIdx = celebChart.planets.Venus ? celebChart.planets.Venus.sign.idx : 0;
                var celebMarsIdx  = celebChart.planets.Mars  ? celebChart.planets.Mars.sign.idx  : 0;
                var celebSunSign  = celebChart.sun.sign;
                var celebMoonSign = celebChart.moon.sign;
                var celebVSign    = celebChart.planets.Venus ? celebChart.planets.Venus.sign.sign : '?';
                var celebMSign    = celebChart.planets.Mars  ? celebChart.planets.Mars.sign.sign  : '?';

                var _signs = typeof astrologer !== 'undefined' ? astrologer.signs : [];

                /* ── 각도(Aspect) 계산: 두 경도 차이 → 주요 각도 판별 ── */
                function _aspect(a, b) {
                    var diff = Math.abs((a - b + 360) % 360);
                    if (diff > 180) diff = 360 - diff;
                    if (diff <= 8)  return { name:'합(Conjunction)', symbol:'☌', nature:'강렬', score:10, color:'#fbbf24' };
                    if (diff >= 52  && diff <= 68)  return { name:'섹스타일(Sextile)', symbol:'⚹', nature:'조화', score:6, color:'#34d399' };
                    if (diff >= 82  && diff <= 98)  return { name:'스퀘어(Square)',   symbol:'□', nature:'도전', score:-5, color:'#f87171' };
                    if (diff >= 112 && diff <= 128) return { name:'트라인(Trine)',    symbol:'△', nature:'완벽조화', score:10, color:'#818cf8' };
                    if (diff >= 172 && diff <= 188) return { name:'어포지션(Opposition)', symbol:'☍', nature:'극단긴장', score:-3, color:'#fb923c' };
                    return null;
                }

                /* ── 주요 시나스트리 각도 쌍 ── */
                var _myLons  = {
                    '내 ☀ 태양':  sunIndex * 30 + (chart.sun.deg  || 0),
                    '내 ☽ 달':    moonIndex * 30+ (chart.moon.deg || 0),
                    '내 ♀ 금성':  (chart.planets.Venus ? chart.planets.Venus.sign.idx * 30 + (chart.planets.Venus.sign.deg || 0) : sunIndex * 30),
                    '내 ♂ 화성':  (chart.planets.Mars  ? chart.planets.Mars.sign.idx  * 30 + (chart.planets.Mars.sign.deg  || 0) : sunIndex * 30)
                };
                var _theirLons = {
                    '상대 ☀ 태양':  celebSunIdx   * 30 + (celebChart.sun.deg || 0),
                    '상대 ☽ 달':    celebMoonIdx  * 30 + (celebChart.moon.deg || 0),
                    '상대 ♀ 금성':  celebVenusIdx * 30 + (celebChart.planets.Venus ? celebChart.planets.Venus.sign.deg || 0 : 0),
                    '상대 ♂ 화성':  celebMarsIdx  * 30 + (celebChart.planets.Mars  ? celebChart.planets.Mars.sign.deg  || 0 : 0)
                };

                var aspectRows = [];
                var totalScore = 0;
                var myKeys  = Object.keys(_myLons);
                var thKeys  = Object.keys(_theirLons);
                for (var mi = 0; mi < myKeys.length; mi++) {
                    for (var ti = 0; ti < thKeys.length; ti++) {
                        var asp = _aspect(_myLons[myKeys[mi]], _theirLons[thKeys[ti]]);
                        if (asp) {
                            aspectRows.push({ pair: myKeys[mi] + ' × ' + thKeys[ti], asp: asp });
                            totalScore += asp.score;
                        }
                    }
                }
                /* 0–100 스코어 정규화 */
                var rawMax = 10 * myKeys.length; // 최대 이론치
                var synScore = Math.min(100, Math.max(0, Math.round(50 + (totalScore / rawMax) * 50)));
                var scoreColor = synScore >= 80 ? '#2ed573' : synScore >= 55 ? '#f39c12' : '#ff4757';

                /* ── 관계 유형 판정 ── */
                var sunDelta = Math.abs((celebSunIdx - sunIndex + 12) % 12);
                if (sunDelta > 6) sunDelta = 12 - sunDelta;
                var relType = sunDelta === 0 ? '영혼의 거울 — 자신을 보는 듯한 완벽한 공명' :
                              sunDelta <= 2 ? '불꽃의 동행 — 에너지가 유사해 폭발적 시너지 발휘' :
                              sunDelta === 3 ? '자극의 파트너 — 서로를 단련시키는 균형의 긴장감' :
                              sunDelta === 4 ? '안정의 동반자 — 성격 결이 달라 서로를 보완하는 최고 궁합' :
                              sunDelta === 5 ? '우주적 반전 — 초반 마찰 후 깊은 교감에 이르는 성장형 인연' :
                              '운명의 대극 — 강렬한 끌림과 충돌이 공존하는 카르마 인연';

                /* ── 섹션별 궁합 해석 ── */
                var LOVE_TABLE = [
                    '마음의 주파수가 완벽히 일치합니다. 처음 만나도 오래된 연인처럼 편안합니다.',
                    '서로의 감정을 직관적으로 이해하며 강한 끌림이 빠르게 형성됩니다.',
                    '불꽃 같은 로맨스. 열정적이지만 충돌이 관계를 더욱 깊게 만듭니다.',
                    '서로의 차이가 오히려 성장의 자극이 되는 강렬한 연애입니다.',
                    '이성적 연결보다 감정적 교감이 더 깊어지는 영적 파트너십입니다.',
                    '처음은 낯설어도 시간이 갈수록 단단해지는 카르마 인연입니다.'
                ];
                var BUSI_TABLE = [
                    '완벽한 협업 — 둘이 함께하면 1+1이 3이 됩니다.',
                    '강한 추진력으로 프로젝트를 이끄는 최적의 파트너.',
                    '창의성과 실행력의 균형, 서로의 약점을 보완합니다.',
                    '논쟁이 있어도 그것이 더 나은 결과를 낳는 생산적 긴장.',
                    '신뢰 기반의 장기 파트너십, 안정적 성장에 적합.',
                    '극복해야 할 과제가 있지만 그것이 더 큰 성과를 만듭니다.'
                ];
                var SPIRIT_TABLE = [
                    '전생에서 이어온 듯한 영혼의 결합, 깊은 직관적 연결.',
                    '서로에게 중요한 영적 교훈을 주는 스승과 제자 같은 관계.',
                    '함께할 때 우주에너지가 정화되는 힐링 파트너.',
                    '영적 성장을 위해 시련을 함께 극복해야 하는 카르마 인연.',
                    '내면의 그림자를 서로에게 투영하며 성장하는 관계.',
                    '만나야 할 이유가 있는, 우주가 안배한 특별한 인연.'
                ];
                var tabIdx = Math.min(5, sunDelta);
                var loveDesc   = LOVE_TABLE[tabIdx];
                var busDesc    = BUSI_TABLE[tabIdx];
                var spiritDesc = SPIRIT_TABLE[tabIdx];

                /* ── 빛과 그림자 키워드 ── */
                var ELEM_MAP = { 0:'불', 1:'불', 2:'흙', 3:'흙', 4:'공기', 5:'공기', 6:'물', 7:'물', 8:'불', 9:'불', 10:'흙', 11:'흙' };
                var myElem = ELEM_MAP[sunIndex % 12] || '균형';
                var theirElem = ELEM_MAP[celebSunIdx % 12] || '균형';
                var SHADOW = {
                ...
                var isUnknownTime = (!birth || true); // 유명인은 항상 생시 미상 처리
                var flag = (typeof COUNTRY_CONFIG !== 'undefined' && COUNTRY_CONFIG[(CELEBS.find(function(x){return x.name===name;})||{}).nationality]) ? COUNTRY_CONFIG[(CELEBS.find(function(x){return x.name===name;})||{}).nationality].flag + ' ' : '';
                ...
                html2 += '<div style="display:flex;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:14px;">'
                    ...
                    + celebSunSign + ' ☀</div>'
                    + (isUnknownTime ? '<div style="font-size:0.65rem;color:#64748b;border:1px dashed #334155;padding:2px 8px;border-radius:10px;">⚠ 생시미상 — 달·상승궁 오차 있음</div>' : '')
                    +'</div>';
                ...
                if (aspectRows.length > 0) {
                    ...
                    aspectRows.slice(0, 8).forEach(function(r) {
                        html2 += '<div style="display:flex;align-items:center;gap:8px;background:rgba(255,255,255,0.03);border-radius:7px;padding:6px 10px;">'
                            ...
                            +'<span style="font-size:0.68rem;background:rgba('+( r.asp.score>0?'52,211,153':'239,68,68' )+',0.15);color:'+r.asp.color+';padding:2px 7px;border-radius:10px;">'+r.asp.name+'</span>'
                            +'</div>';
                    });
                    html2 += '</div></div>';
                }
    ...
    window._astroDirectSynastry = function() {
        ...
        setTimeout(function() {
            try {
                ...
                var partnerChart = AstroEngine.calcAll(+dp[0], +dp[1], +dp[2], localHour, 37.6, 127.0, tzVal);

                var pSunIdx   = partnerChart.sun.idx;
                var pMoonIdx  = partnerChart.moon.idx;
                var pVenusIdx = partnerChart.planets.Venus ? partnerChart.planets.Venus.sign.idx : pSunIdx;
                var pMarsIdx  = partnerChart.planets.Mars  ? partnerChart.planets.Mars.sign.idx  : pSunIdx;
                var pSunSign  = partnerChart.sun.sign;
                var pMoonSign = partnerChart.moon.sign;
                var pVSign    = partnerChart.planets.Venus ? partnerChart.planets.Venus.sign.sign : '?';
                var pMSign    = partnerChart.planets.Mars  ? partnerChart.planets.Mars.sign.sign  : '?';
                ...
                var _pL = {
                    '상대 ☀ 태양': pSunIdx   * 30 + (partnerChart.sun.deg  || 0),
                    '상대 ☽ 달':   pMoonIdx  * 30 + (partnerChart.moon.deg || 0),
                    '상대 ♀ 금성': pVenusIdx * 30 + (partnerChart.planets.Venus ? partnerChart.planets.Venus.sign.deg || 0 : 0),
                    '상대 ♂ 화성': pMarsIdx  * 30 + (partnerChart.planets.Mars  ? partnerChart.planets.Mars.sign.deg  || 0 : 0)
                };

                var aspectRows2 = [];
                var totalScore2 = 0;
                var myKeys2 = Object.keys(_myL);
                var pKeys2  = Object.keys(_pL);
                for (var mi = 0; mi < myKeys2.length; mi++) {
                    for (var pi = 0; pi < pKeys2.length; pi++) {
                        var asp2 = _asp2(_myL[myKeys2[mi]], _pL[pKeys2[pi]]);
                        if (asp2) {
                            aspectRows2.push({ pair: myKeys2[mi] + ' × ' + pKeys2[pi], asp: asp2 });
                            totalScore2 += asp2.score;
                        }
                    }
                }
                var synScore2 = Math.min(100, Math.max(0, Math.round(50 + (totalScore2 / (10 * myKeys2.length)) * 50)));
                var scoreColor2 = synScore2 >= 80 ? '#2ed573' : synScore2 >= 55 ? '#f39c12' : '#ff4757';

                var sunDelta2 = Math.abs((pSunIdx - sunIndex + 12) % 12);
                if (sunDelta2 > 6) sunDelta2 = 12 - sunDelta2;
                var relType2 = sunDelta2 === 0 ? '영혼의 거울 — 자신을 보는 듯한 완벽한 공명' :
                               sunDelta2 <= 2 ? '불꽃의 동행 — 에너지가 유사해 폭발적 시너지 발휘' :
                               sunDelta2 === 3 ? '자극의 파트너 — 서로를 단련시키는 균형의 긴장감' :
                               sunDelta2 === 4 ? '안정의 동반자 — 성격 결이 달라 서로를 보완하는 최고 궁합' :
                               sunDelta2 === 5 ? '우주적 반전 — 초반 마찰 후 깊은 교감에 이르는 성장형 인연' :
                               '운명의 대극 — 강렬한 끌림과 충돌이 공존하는 카르마 인연';
                ...
                var tabIdx2 = Math.min(5, sunDelta2);

                var ELEM_MAP2 = { 0:'불', 1:'불', 2:'흙', 3:'흙', 4:'공기', 5:'공기', 6:'물', 7:'물', 8:'불', 9:'불', 10:'흙', 11:'흙' };
                var myElem2   = ELEM_MAP2[sunIndex % 12] || '균형';
                var theirElem2 = ELEM_MAP2[pSunIdx % 12] || '균형';
                var SHADOW2 = {
                    '불-불':     { light:'✧ 불꽃 같은 열정과 에너지가 증폭',     shadow:'✦ 둘 다 리더 기질, 주도권 충돌 주의',        remedy:'서로의 에너지를 경쟁이 아닌 창조로 승화시키세요' },
                    '불-흙':     { light:'✧ 열정+현실감각의 이상적 조합',         shadow:'✦ 속도 차이 — 불은 빠르고 흙은 느립니다',     remedy:'페이스 조율: 행동 전 충분한 논의가 신뢰를 만듭니다' },
                    '불-공기':   { light:'✧ 창의적 영감이 폭발하는 관계',         shadow:'✦ 감정보다 언어, 피상적 교류에 머물 수 있음', remedy:'진심을 담은 깊은 대화 시간을 의도적으로 만드세요' },
                ...
                h += '<div style="display:grid;grid-template-columns:1fr;gap:6px;margin-bottom:12px;">'
                    +'<div style="background:rgba(244,114,182,0.08);border-radius:10px;padding:10px 12px;border:1px solid rgba(244,114,182,0.2);">'
                    +'<div style="font-size:0.72rem;color:#f472b6;font-weight:700;margin-bottom:4px;">💕 연애 궁합</div>'
                    +'<p style="font-size:0.82rem;color:#e2e8f0;margin:0;line-height:1.6;word-break:keep-all;">'+LOVE_T2[tabIdx2]+'</p></div>'
                    +'<div style="background:rgba(251,191,36,0.07);border-radius:10px;padding:10px 12px;border:1px solid rgba(251,191,36,0.18);">'
                    +'<div style="font-size:0.72rem;color:#fbbf24;font-weight:700;margin-bottom:4px;">🤝 비즈니스 궁합</div>'
                    +'<p style="font-size:0.82rem;color:#e2e8f0;margin:0;line-height:1.6;word-break:keep-all;">'+BUSI_T2[tabIdx2]+'</p></div>'
                    +'<div style="background:rgba(129,140,248,0.07);border-radius:10px;padding:10px 12px;border:1px solid rgba(129,140,248,0.18);">'
                    +'<div style="font-size:0.72rem;color:#818cf8;font-weight:700;margin-bottom:4px;">✨ 영적 궁합</div>'
                    +'<p style="font-size:0.82rem;color:#e2e8f0;margin:0;line-height:1.6;word-break:keep-all;">'+SPIRIT_T2[tabIdx2]+'</p></div>'
                    +'</div>';
...
var mainSt = extractMains(getPStars(pName));
        ...
        var sec_grand = '<div style="background:linear-gradient(135deg,rgba(88,28,220,0.15),rgba(20,10,50,0.8));padding:18px;border-radius:10px;margin-bottom:20px;border:1px solid rgba(139,92,246,0.3);">'
          ...
          +'</div>'
          +'<div style="font-size:0.9rem;line-height:1.75;color:#e2e8f0;">'
            +'<b style="color:#f9a8d4;">생애 기질:</b> '+lifeSentence+'<br>'
            +'<b style="color:#f9a8d4;">起運 나이:</b> <span style="color:#fbbf24;">'+juVal+'세</span>부터 대한이 '+dirText+'으로 펼쳐집니다.<br>'
            +(sihuaSummary.length>0?'<b style="color:#f9a8d4;">사화(四化) 배치:</b> '+sihuaSummary.join(' &nbsp;·&nbsp; '):'')
          +'</div>'
        +'</div>';
        ...
        var PALACE_DAHAN_THEME = {
          ...
          '노복궁':{icon:'👥',kw:'부하·조직·핵심 인맥 관리가 성패를 가름'},
        ...
        if (pd.daHanList && pd.sihuaData) {
          pd.daHanList.forEach(function(dh) {
            ...
            for (var shS2 in pd.sihuaData) {
              if (pd.sihuaData[shS2].palaceIdx === dh.idx) dhSihua2.push({star:shS2, info:pd.sihuaData[shS2]});
            }
            var hwagiList  = dhSihua2.filter(function(s){return s.info.type==='화기';});
            var hwarocList = dhSihua2.filter(function(s){return s.info.type==='화록';});
            if (hwagiList.length>0 && ['재백궁','관록궁','부처궁','명궁','천이궁'].indexOf(dh.palaceName)>=0) {
              pivots.push({age:dh.startAge+'~'+dh.endAge,type:'crisis',icon:'⚠️',
                title:dh.startAge+'세 대한: '+dh.palaceName+' — 화기(忌) 진입',
                desc:hwagiList[0].star+' 화기(忌)가 <b>'+dh.palaceName+'</b>에 진입합니다. 재물·커리어·파트너십 등 핵심 영역에서 균열과 변동이 집약됩니다. 신중한 의사결정과 리스크 최소화가 필수입니다.'});
            }
            if (hwarocList.length>0 && ['재백궁','관록궁','명궁'].indexOf(dh.palaceName)>=0) {
              pivots.push({age:dh.startAge+'~'+dh.endAge,type:'chance',icon:'⭐',
                title:dh.startAge+'세 대한: '+dh.palaceName+' — 화록(祿) 활성',
                desc:hwarocList[0].star+' 화록(祿)이 <b>'+dh.palaceName+'</b>에 활성화됩니다. 수입·기회·귀인의 흐름이 집중되는 전성기입니다. 이 10년을 가장 적극적으로 행동해야 합니다.'});
            }
          });
        }
        if (pivots.length < 2 && pd.daHanList) {
          var strongSt = ['자미','칠살','파군','태양','무곡'];
          pd.daHanList.forEach(function(dh2) {
            var dhM2 = extractMains(pd.stars[dh2.idx]);
            if (dhM2.some(function(s){return strongSt.indexOf(s)>=0;}) && dh2.palaceName==='명궁') {
              pivots.push({age:dh2.startAge+'~'+dh2.endAge,type:'turn',icon:'🔀',
                title:dh2.startAge+'세 대한: 명궁 강성(强星) 대한',
                desc:'<b>'+dhM2[0]+'</b>이 명궁에 진입하는 자아 전환 대한입니다. 정체성 재정립 또는 생애 방향의 대전환이 발생하기 쉬운 핵심 10년입니다.'});
            }
          });
        }
        var pivotHtml = '';
        var shownPivots = pivots.slice(0,3);
        if (shownPivots.length>0) {
          shownPivots.forEach(function(p){
            var bc = p.type==='crisis'?'#f87171':(p.type==='chance'?'#4ade80':'#a78bfa');
            pivotHtml += '<div style="border-left:4px solid '+bc+';padding:12px 14px;background:'+bc+'11;border-radius:0 8px 8px 0;margin-bottom:10px;">'
              +'<div style="color:'+bc+';font-weight:800;font-size:0.92rem;margin-bottom:4px;">'+p.icon+' '+p.title+'</div>'
              +'<div style="color:#e2e8f0;font-size:0.84rem;line-height:1.6;">'+p.desc+'</div>'
            +'</div>';
          });
        } else {
...
function calcSukuyoData(lunarObj, opt = { leapRule: 'current' }) {
    ...
    const baseData = {
        ...
        "방": {icon:"🌠",celebs:"일론 머스크, 제프 베이조스",talent:92,
            core:"방수(房宿)는 동방 청룡의 심장을 뜻합니다. 하늘의 마부좌로 불리며, 당당함과 권위, 그리고 성공을 향한 불꽃을 가슴에 품고 태어납니다. 방수 아래 태어난 영혼은 이 세상에서 가시적인 성취와 명예를 빛내기 위해 왔습니다. 타고난 전략가이자 승부사로, 어떤 상황에서도 기회를 포착하는 날카로운 눈을 지녔습니다.",
        ...
        "여": {icon:"🌙",celebs:"셰익스피어, 톨스토이",talent:93,
            core:"여수(女宿)는 직녀성으로도 알려진 완벽주의와 절제의 별입니다. 여수 아래 태어난 영혼은 하늘의 베를 짜는 장인처럼, 자신의 세계를 정밀하게 설계하고 완성하기 위해 이 땅에 왔습니다. 철저한 자기 관리와 품위를 삶의 기준으로 삼으며, 어떤 환경에서도 자신만의 격을 잃지 않는 귀한 영혼입니다.",
            ...
            desc:"철저한 자기 관리와 완벽주의의 기운입니다. 내성적이지만 권위와 체면을 깊게 중시합니다.",
        ...
        "위1": {icon:"🔥",celebs:"알렉산더 대왕, 율리우스 카이사르",talent:89,char:"危",
            ...
            work:"리스크가 클수록 역량이 폭발하며 큰 성공을 이루는 모 아니면 도 타입입니다.",
        ...
        "실": {icon:"💧",celebs:"이순신, 세종대왕",talent:99,
            ...
            hidden:"실수는 모든 걸 시작하는 기세 뒤에 지속과 마무리에 대한 두려움을 감추고 있습니다. 시작의 에너지는 폭발적이지만 끝을 보지 못하거나 루틴이 생기면 흥미를 잃는 경향이 있습니다. 이 패턴을 스스로 인식하고, 끝맺음을 함께 할 파트너나 시스템을 곁에 두는 것이 성공의 핵심 전략입니다.",
        ...
        "규": {icon:"🌺",celebs:"콜럼버스, 마젤란",talent:85,
            ...
            work:"전통적인 가치나 가업을 계승하거나 관리 감독하는 보수적인 일에 최적화되어 있습니다.",
        ...
        "위2": {icon:"🍁",celebs:"링컨, 처칠",talent:92,char:"胃",
            ...
            hidden:"위수는 모든 것을 분석하고 파악하려는 욕구 때문에 머리가 쉬지 못하는 번뇌를 겪습니다. 지나친 분석과 전략적 사고로 인해 가끔 단순하고 즉흥적인 행복을 놓칩니다. 또한 식욕이나 지식 욕구 등 뭔가를 축적하고 채우려는 충동을 통제하는 것이 평생 과제 중 하나입니다.",
        ...
        "정": {icon:"🌪️",celebs:"마리 앙투아네트, 볼테르",talent:84,
            core:"정수(井宿)는 하늘의 우물을 의미하는 별로, 차갑고 청명한 이성의 원천을 상징합니다. 정수 아래 태어난 영혼은 세상의 모든 것을 논리와 합리의 눈으로 분류하고 체계화하기 위해 왔습니다. 냉정한 분석력으로 진실을 가려내는 탐정이자, 시스템의 비효율을 가장 먼저 발견하는 눈을 가졌습니다.",
      ...
      if (D === 0) {
        ...
        base = {
          ...
          advantages: [
            ...
            { icon: '💼', label: '현실적 보완', text: '완벽한 역할 분담이 이뤄진다. ' + (isDriving ? '당신의 방향 설정 능력과 상대의 실행력이 결합하여 1+1=11이 되는 시너지.' : '상대의 전략과 당신의 디테일한 실행이 맞물려 최고의 결과물을 만든다.') },
