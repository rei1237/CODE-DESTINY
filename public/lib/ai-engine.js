(function () {
  ...
  function interpretDream(inputText) {
    ...
    var step1 = {
      ...
      meaning: '당신의 꿈속 안개를 걷어내니 첫 번째 카드 <' + rootName + '>가 모습을 드러냅니다. 이 카드는 "' + parsed.subject.key + '"가 품은 ' + (parsed.subject.energy || '핵심 본능') + '의 결을 비춰, 지금 마음이 무엇을 지키려 하는지 먼저 들려줍니다. ' + chemistry + ' 지금 필요한 건 감정을 억누르는 힘이 아니라, 흔들리는 이유를 정확히 이름 붙이는 용기입니다.',
      warning_or_healing: '치유 루틴: 꿈 장면을 [대상-행동-감정] 3줄로 적고, 오늘 하루 내가 지킬 경계 1가지만 정하면 흐름이 빠르게 안정됩니다.'
    };
    ...
    var step2 = {
      ...
      meaning: '두 번째 카드 <' + messageName + '>는 책장 사이로 금빛 먼지를 흩뿌리며 "' + parsed.action.key + '"라는 장면을 다시 보여줍니다. 현재 당신의 에너지는 ' + parsed.action.vibe + ' 구간에 들어와 있어, 속도보다 순서를 맞출 때 운의 마찰이 줄어듭니다. 즉흥 반응 대신 관찰→선택→행동의 리듬을 붙이면, 막혀 보이던 길이 생각보다 빨리 열립니다.',
    ...
    var step3 = {
      ...
      meaning: '마지막 카드 <' + guidanceName + '>가 조용히 떠오르며 "' + parsed.emotion.key + '" 감정이 사실은 ' + parsed.emotion.tone + ' 신호였다고 말해줍니다. 이 감정은 당신이 약해서 생긴 균열이 아니라, 삶의 속도를 다시 맞추라는 섬세한 신호입니다. 오늘 밤 이 메시지를 받아들이면 내일의 선택은 더 가볍고 단단해집니다.',
      final_comfort: '마법책의 마지막 문장: ' + parsed.emotion.comfort
    };
