(function () {
  ...
  function interpretDream(inputText) {
    ...
    var step1 = {
      step: 1,
      title: '🛡️ 당신의 운명적 에너지 (동물)',
      card_id: rootCard.id || 'root-card',
      card_name: rootCard.name_kr || rootCard.name || '카드',
      meaning: '"' + parsed.subject.key + '" 상징은 ' + (parsed.subject.energy || '핵심 본능') + '을 뜻합니다. 꿈의 뿌리 에너지는 당신이 이미 회복의 출발점을 알고 있음을 보여줍니다. ' + chemistry,
      warning_or_healing: '실질적 이점: 불안한 대상을 피하지 않고 구조화하면 관계와 선택에서 주도권을 되찾습니다.'
    };
    ...
    var step2 = {
      ...
      meaning: '"' + parsed.action.key + '" 행동은 현재 ' + parsed.action.vibe + ' 국면임을 말합니다. 행동 방식 교정이 곧 성취도 상승으로 이어집니다.',
    ...
    var step3 = {
      ...
      meaning: '"' + parsed.emotion.key + '" 감정은 ' + parsed.emotion.tone + ' 신호입니다. 감정은 문제 자체가 아니라 리스크 관리 타이밍을 알려주는 알림입니다.',
    ...
    var payload = {
      wizard_greeting: '여행자여, 당신의 무의식 속에 숨겨진 세 개의 열쇠를 찾았습니다.',
      analysis: {
    ...
    return {
      ...
      summary: payload.wizard_greeting + ' ' + chemistry,
      ...
      finalSpell: '오늘의 주문: 나는 감정을 신호로 읽고, 행동을 전략으로 바꾼다.',
      json: payload,
